...
在批量导出文件夹中的文件时，您可以选择在输出目录中保持原始文件夹结构。在导出对话框中启用"保持文件夹结构"选项，以保持与您的库相同的目录层次结构。当您希望导出的 PDF 文件以与 Markdown 文件相同的方式组织时，这非常有用。

- **启用时**：文件导出到与您的库结构匹配的子目录中（例如：`文件夹/子文件夹/文件.pdf`）
- **禁用时**：所有文件都导出到平面目录结构中（默认行为）

#### 复制非Markdown文件

导出文件夹时，您还可以复制非Markdown文件（图片、PDF、附件等）与导出的PDF一起保存。在导出对话框中启用"复制非Markdown文件"选项，可以将所有文件复制到输出目录。

- **启用时**：非Markdown文件被复制到输出目录与导出的PDF一起保存
- **禁用时**：仅创建PDF导出（默认行为）

当您的Markdown文件引用图片或其他资源，并且您希望将它们与导出的PDF保存在一起时，此功能特别有用。
